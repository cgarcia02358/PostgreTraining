...
INSERT INTO customers (first_name, last_name) VALUES ('Carlo', 'Garcia');
...
DECLARE
  ...
  current_balance NUMERIC;
  new_balance NUMERIC;
  _maint_balance NUMERIC;


BEGIN
  SELECT balance INTO current_balance FROM accounts WHERE accounts.id = _id;
  SELECT maintaining_balance INTO _maint_balance FROM accounts WHERE accounts.id = _id;

  new_balance := current_balance - _amount;

  -- set result to FALSE if new_balance < 0
  IF new_balance < _maint_balance THEN
    result := FALSE;
    RAISE NOTICE 'Account % has will have below maintaining balance of % for withdrawal amount %', _id, _maint_balance, _amount;
  ELSE
    UPDATE accounts SET balance = new_balance WHERE accounts.id = _id;
  END IF;
